...
const { t } = useI18n();
const props = defineProps<{ label: string; listQuery: any; queryKey: string; searchConfig: any; defaultViewType?: string; refetchNeeded?: boolean }>();
                          ...
                          <div v-for="item in data[queryKey].edges" :key="item.node.id" class="file-entry relative">
                                ...
                                <template v-if="item.node.type === TYPE_IMAGE">
                                  <Link :path="getPath(item.node)">
                                    <Image :source="item.node.imageWebUrl" alt="File thumbnail" class="w-full h-48"/>
                                  </Link>
                                </template>
