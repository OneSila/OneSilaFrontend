  ...
  <div>
    <Modal v-model="localShowModal" @closed="closeModal">
      <Card class="modal-content w-2/3">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            ...
            <div class="mt-1 w-full" >
                <Flex vertical>
                  ...
                  <FlexCell>
                    <WebsiteInput class="w-full" v-model:model-value="websites[index]" @update:model-value="updateWebsite(index, $event)"/>
                  </FlexCell>
                </Flex>
            </div>
          </div>
        ...
        <ApolloMutation :mutation="createVideosMutation" :variables="{ data: websites.map(url => ({ videoUrl: url })) }"  @done="onVideosCreated" @error="onError">
          <template v-slot="{ mutate, loading, error }">
            <div class="flex justify-end gap-4 mt-4">
              <Button class="btn btn-outline-dark" @click="closeModal">{{ t('shared.button.cancel') }}</Button>
              <Button class="btn btn-primary" :disabled="loading || websites.filter(Boolean).length === 0" @click="mutate">{{ t('shared.button.submit') }}</Button>
            </div>
          </template>
        </ApolloMutation>
      </Card>
