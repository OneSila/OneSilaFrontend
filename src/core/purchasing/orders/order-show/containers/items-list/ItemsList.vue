...
<template>
  <GeneralTemplate>
    ...
    <template v-slot:buttons>
          <Link :path="{ name: 'purchasing.orders.items.create', params: {orderId: id} }">
          <Button class="btn btn-primary">
              {{  t('purchasing.orders.items.create.button') }}
          </Button>
        </Link>
    </template>
   ...
   <template v-slot:content>
      <GeneralListing
        ...
        :fixed-filter-variables="{'purchaseOrder': {'id': {'exact': id}}}"
    />
   </template>
  </GeneralTemplate>
</template>