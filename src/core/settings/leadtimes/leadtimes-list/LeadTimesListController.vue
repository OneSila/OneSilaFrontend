...
<template>
  <SettingsTemplate>
    ...
    <template v-slot:breadcrumbs>
      <Breadcrumbs :links="[{ path: { name: 'settings.leadtimes.list' }, name: t('settings.leadtimes.title') }]" />
    </template>

    <template v-slot:buttons>
        <div>
          <Link :path="{ name: 'settings.leadtimes.create' }">
            <Button type="button" class="btn btn-primary">
                {{  t('settings.leadtimes.create.title') }}
            </Button>
          </Link>
        </div>
    </template>
