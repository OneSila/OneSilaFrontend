...
import {
  ...
  magentoRemoteAttributeSetsQuery,
...
import {
  ...
  updateMagentoSalesChannelMutation,
  createRemoteEanCodeAttributeMutation,
  updateSalesChannelImportMutation
} from "../../../../../../../../shared/api/mutations/salesChannels.js";
import apolloClient from "../../../../../../../../../apollo-client";

import { LanguagesAndCurrenciesStep } from "./containers/languages-and-currencies";
import { AttributesStep } from "./containers/attributes";
import { DefaultRuleStep } from "./containers/default-rule";
...
const fetchIntegrationData = async () => {
  try {
    ...
    const { data } = await apolloClient.query({
      ...
      variables: { id: integrationId.value },
...
const fetchAttributesData = async () => {
  try {
    const { data } = await apolloClient.query({
      ...
      variables: { salesChannelId: integrationId.value },
      fetchPolicy: 'network-only',
    });
...
const bulkUpdateRemoteLanguages = async () => {
  ...
  try {
    const {data} = await apolloClient.mutate({
      ...
      variables: {
        data: mappedData.value.languages
      },
    });
    ...
    const { data } = await apolloClient.mutate({
      ...
      variables: {
        data: {
            ...
            salesChannel: { id: integrationId.value },
        },
      },
    });
...
const handleFinish = async () => {
  ...
  try {
    ...
    router.push({ name: 'integrations.integrations.show', params: { id: integrationId.value, type: type.value }, query: { tab: 'imports' } });
  } catch (err) {
