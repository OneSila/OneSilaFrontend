...
const handleSetData = (data: any) => {
  const propertyData = data?.amazonProperty;
  if (propertyData && !propertyData.originalType) {
    propertyData.originalType = propertyData.type;
  }

  const currentType = propertyData?.type;
  if (!formConfig.value || !currentType) {
    return;
  }
