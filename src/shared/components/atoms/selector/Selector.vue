...
const props = withDefaults(
  ...
  { options: [] as any, dropdownPosition: 'top', removable: true },
);
...
const onAddClicked = () => {
  const searchEl = selectorRef.value.searchEl;
  if (searchEl) {
    searchEl.blur();
  }
...
onMounted(() => {
  if (props.openOnEmpty && (!props.modelValue || !props.modelValue.length)) {
    selectorRef.value?.searchEl.focus();
  }
});
...
<template>
  <VueSelect
    ref="selectorRef"
    class="selector min-w-38 text-sm"
    :class="{
  ...
  >
    <template #no-options>
      <div class="w-full px-2 text-left" style="opacity: 0.5">
        <em>{{ t('shared.components.molecules.selector.typeToSearch') }}.</em>
      </div>
    </template>

    <template v-if="addable" #list-footer>
      <div class="border-t-2 border-gray-300 p-1">
        <Button
          class="h-full w-full rounded hover:bg-blue-400 hover:text-white"
          @click="onAddClicked"
        >
          Add
        </Button>
    ...
    <template v-else-if="more" #list-footer>
      <li style="opacity: 0.5" class="px-5">
        {{ t('shared.components.molecules.selector.moreAvailable') }}
      </li>
    </template>
  </VueSelect>
</template>
