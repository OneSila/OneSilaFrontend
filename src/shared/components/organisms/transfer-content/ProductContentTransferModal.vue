<script setup lang="ts">
import { computed, reactive, ref, watch } from 'vue';
import { useI18n } from 'vue-i18n';
import apolloClient from '../../../../../apollo-client';
import { Button } from '../../atoms/button';
import { Icon } from '../../atoms/icon';
import { Modal } from '../../atoms/modal';
import { Card } from '../../atoms/card';
import { IntegrationsSelector } from '../../molecules/integrations-selector';
import { Toggle } from '../../atoms/toggle';
import { Badge } from '../../atoms/badge';
import { Accordion } from '../../atoms/accordion';
import { Checkbox } from '../../atoms/checkbox';
import { Selector } from '../../atoms/selector';
import { Toast } from '../../../modules/toast';
import { formatIntegrationLabel, processGraphQLErrors } from '../../../utils';
import {
  getProductContentsByChannelQuery,
  getProductContentsByDefaultQuery,
  productTranslationBulletPointsQuery,
} from '../../../api/queries/products.js';
import {
  createProductTranslationMutation,
  importProductTranslationsMutation,
  updateProductTranslationMutation,
  createProductTranslationBulletPointsMutation,
  deleteProductTranslationBulletPointsMutation,
} from '../../../api/mutations/products.js';
import { translationLanguagesQuery } from '../../../api/queries/languages.js';
import { getContentFieldRules } from '../../../../core/products/products/product-show/containers/tabs/content/contentFieldRules';
import ProductContentPreview from '../../../../core/products/products/product-show/containers/tabs/content/ProductContentPreview.vue';

interface SalesChannel {
  id: string;
  hostname?: string | null;
  type?: string | null;
}

interface TranslationNode {
  id: string;
  language: string;
  name?: string | null;
  subtitle?: string | null;
  shortDescription?: string | null;
  description?: string | null;
  urlKey?: string | null;
}

interface BulletPointNode {
  id?: string | null;
  text?: string | null;
  sortOrder?: number | null;
}

type TransferMode = 'import' | 'export';
type TransferFieldKey = 'name' | 'subtitle' | 'shortDescription' | 'description' | 'urlKey' | 'bulletPoints';
type ContentTextFieldKey = Exclude<TransferFieldKey, 'bulletPoints'>;

interface LanguageOption {
  code: string;
  name?: string | null;
}

interface Props {
  productIds: string[];
  currentLanguage: string | null;
  currentSalesChannel: 'default' | string | null;
  salesChannels: SalesChannel[];
  btnClass?: string;
  useDefaultButtonStyles?: boolean;
  buttonLabel?: string;
  iconName?: string | null;
  iconClass?: string;
}

const props = withDefaults(defineProps<Props>(), {
  btnClass: 'btn-outline-primary mx-2',
  useDefaultButtonStyles: true,
  buttonLabel: 'products.translation.transfer.button',
  iconName: null,
  iconClass: 'mr-2',
});

const emit = defineEmits<{
  (e: 'transferred'): void;
  (e: 'imported'): void;
}>();

const { t } = useI18n();

const emptyHtml = '';
const transferFieldKeys: TransferFieldKey[] = ['name', 'subtitle', 'shortDescription', 'description', 'urlKey', 'bulletPoints'];
const textFieldKeys: ContentTextFieldKey[] = ['name', 'subtitle', 'shortDescription', 'description', 'urlKey'];
const emptyFieldAvailability: Record<TransferFieldKey, boolean> = {
  name: false,
  subtitle: false,
  shortDescription: false,
  description: false,
  urlKey: false,
  bulletPoints: false,
};

const hasProducts = computed(() => props.productIds.length > 0);
const buttonClass = computed(() => {
  if (props.useDefaultButtonStyles) {
    return `btn ${props.btnClass}`.trim();
  }
  return props.btnClass;
});

const showModal = ref(false);
const transferMode = ref<TransferMode>('import');
const selectedSourceChannel = ref<string | null>(null);
const selectedTargetChannel = ref<string | null>(null);
const selectedTargetChannels = ref<string[]>([]);
const selectedLanguage = ref<string | null>(null);
const overrideExisting = ref(false);
const transferAllLanguages = ref(false);
const loading = ref(false);
const transferring = ref(false);
const languageLoading = ref(false);
const languageOptions = ref<LanguageOption[]>([]);
const sourceTranslations = ref<TranslationNode[]>([]);
const sourcePreview = ref<TranslationNode | null>(null);
const sourcePreviewBulletPoints = ref<BulletPointNode[]>([]);
const destinationTranslations = ref<TranslationNode[]>([]);
const destinationPreview = ref<TranslationNode | null>(null);
const destinationPreviewBulletPoints = ref<BulletPointNode[]>([]);
const selectedFields = reactive<Record<TransferFieldKey, boolean>>({
  name: false,
  subtitle: false,
  shortDescription: false,
  description: false,
  urlKey: false,
  bulletPoints: false,
});
const fieldEnumMap: Record<TransferFieldKey, string> = {
  name: 'NAME',
  subtitle: 'SUBTITLE',
  shortDescription: 'SHORT_DESCRIPTION',
  description: 'DESCRIPTION',
  urlKey: 'URL_KEY',
  bulletPoints: 'BULLET_POINTS',
};

const formatChannelLabel = (channel?: SalesChannel) => {
  if (!channel) return t('shared.labels.default');
  return formatIntegrationLabel(channel) || t('shared.labels.default');
};

const channelLabel = (channelId: string | null) => {
  if (!channelId || channelId === 'default') {
    return t('shared.labels.default');
  }
  const channel = props.salesChannels.find((item) => item.id === channelId);
  return channel ? formatChannelLabel(channel) : channelId;
};

const isSingleUpdate = computed(() => props.productIds.length === 1);
const resolvedProductId = computed(() => (isSingleUpdate.value ? props.productIds[0] : null));
const resolvedLanguage = computed(() => props.currentLanguage ?? selectedLanguage.value);
const sourceChannel = computed(() => {
  if (transferMode.value === 'export') {
    return props.currentSalesChannel ?? selectedSourceChannel.value;
  }
  return selectedSourceChannel.value;
});
const importTargetChannel = computed(() => props.currentSalesChannel ?? selectedTargetChannel.value);
const targetChannels = computed(() => {
  if (transferMode.value === 'import') {
    return importTargetChannel.value ? [importTargetChannel.value] : [];
  }
  return selectedTargetChannels.value;
});
const previewTargetChannel = computed(() => targetChannels.value[0] || null);
const hasSourceSelection = computed(() => !!sourceChannel.value);
const hasTargetSelection = computed(() => targetChannels.value.length > 0);
const hasLanguageSelection = computed(() => !!resolvedLanguage.value);

const modeOptions = computed<Array<{ key: TransferMode; icon: string; title: string; description: string; detail: string }>>(() => [
  {
    key: 'import',
    icon: 'file-import',
    title: t('products.translation.transfer.modes.import.title'),
    description: t('products.translation.transfer.modes.import.description'),
    detail: t('products.translation.transfer.modes.import.detail'),
  },
  {
    key: 'export',
    icon: 'paper-plane',
    title: t('products.translation.transfer.modes.export.title'),
    description: t('products.translation.transfer.modes.export.description'),
    detail: t('products.translation.transfer.modes.export.detail'),
  },
]);

const countSelectableChannels = (excludeIds: string[], addDefault: boolean) => {
  const exclusionSet = new Set(excludeIds);
  const channelCount = props.salesChannels.filter((channel) => !exclusionSet.has(channel.id)).length;
  return channelCount + (addDefault && !exclusionSet.has('default') ? 1 : 0);
};

const sourceExcludeIds = computed(() => {
  if (transferMode.value === 'import') {
    return importTargetChannel.value ? [importTargetChannel.value] : [];
  }
  return selectedTargetChannels.value;
});
const sourceAddDefault = computed(() => {
  if (transferMode.value === 'import') {
    return importTargetChannel.value !== 'default';
  }
  return !selectedTargetChannels.value.includes('default');
});
const sourceChannelCount = computed(() => countSelectableChannels(sourceExcludeIds.value, sourceAddDefault.value));
const targetExcludeIds = computed(() => (sourceChannel.value ? [sourceChannel.value] : []));
const targetAddDefault = computed(() => sourceChannel.value !== 'default');
const targetChannelCount = computed(() => countSelectableChannels(targetExcludeIds.value, targetAddDefault.value));

const sourceChannelLabel = computed(() => channelLabel(sourceChannel.value));
const targetChannelLabel = computed(() => channelLabel(importTargetChannel.value));
const previewTargetLabel = computed(() => channelLabel(previewTargetChannel.value));
const currentLanguageBadge = computed(() =>
  t('products.translation.transfer.currentLanguageBadge', { code: resolvedLanguage.value || '-' }),
);
const availableLanguagesBadge = computed(() =>
  t('products.translation.transfer.availableLanguagesBadge', { count: sourceTranslations.value.length }),
);
const exportTargetsBadge = computed(() =>
  t('products.translation.transfer.exportTargetsBadge', { count: selectedTargetChannels.value.length }),
);

const resolveChannelType = (channelId: string | null) => {
  if (!channelId || channelId === 'default') return 'default';
  const channel = props.salesChannels.find((item) => item.id === channelId);
  return channel?.type || 'default';
};

const sourceChannelType = computed(() => resolveChannelType(sourceChannel.value));
const destinationChannelTypes = computed(() => targetChannels.value.map((channelId) => resolveChannelType(channelId)));
const sourceRules = computed(() => getContentFieldRules(sourceChannelType.value));
const destinationRules = computed(() => destinationChannelTypes.value.map((channelType) => getContentFieldRules(channelType)));
const previewDestinationRules = computed(() => getContentFieldRules(resolveChannelType(previewTargetChannel.value)));

const baseAllowedFields = computed<Record<TransferFieldKey, boolean>>(() => {
  if (!hasSourceSelection.value || !hasTargetSelection.value) {
    return { ...emptyFieldAvailability };
  }

  return transferFieldKeys.reduce((accumulator, field) => {
    accumulator[field] = Boolean(sourceRules.value[field] && destinationRules.value.every((rules) => rules[field]));
    return accumulator;
  }, { ...emptyFieldAvailability });
});

const allowedFields = computed<Record<TransferFieldKey, boolean>>(() =>
  transferFieldKeys.reduce((accumulator, field) => {
    accumulator[field] = baseAllowedFields.value[field] && selectedFields[field];
    return accumulator;
  }, { ...emptyFieldAvailability }),
);

const advancedOptionsItems = computed(() => [
  { name: 'fields', label: t('products.translation.transfer.advancedOptionsTitle') },
]);

const fieldOptions = computed<Array<{ key: TransferFieldKey; label: string; available: boolean }>>(() => [
  { key: 'name', label: t('shared.labels.name'), available: baseAllowedFields.value.name },
  { key: 'subtitle', label: t('products.translation.labels.subtitle'), available: baseAllowedFields.value.subtitle },
  {
    key: 'shortDescription',
    label: t('shared.labels.shortDescription'),
    available: baseAllowedFields.value.shortDescription,
  },
  { key: 'description', label: t('products.translation.labels.description'), available: baseAllowedFields.value.description },
  { key: 'urlKey', label: t('products.translation.labels.urlKey'), available: baseAllowedFields.value.urlKey },
  { key: 'bulletPoints', label: t('products.translation.labels.bulletPoints'), available: baseAllowedFields.value.bulletPoints },
]);

const selectedFieldEnums = computed(() =>
  (Object.entries(allowedFields.value) as Array<[TransferFieldKey, boolean]>)
    .filter(([, enabled]) => enabled)
    .map(([field]) => fieldEnumMap[field]),
);
const hasSelectedFields = computed(() => selectedFieldEnums.value.length > 0);

const previewChannel = computed(() => previewTargetChannel.value || 'default');
const previewTitle = computed(() => {
  if (transferMode.value === 'export' && targetChannels.value.length > 1 && previewTargetChannel.value) {
    return t('products.translation.transfer.previewTitleWithTarget', { target: previewTargetLabel.value });
  }
  return t('products.translation.transfer.previewTitle');
});

const showLanguageSelector = computed(() => props.currentLanguage === null && !transferAllLanguages.value);
const allLanguagesHelp = computed(() => (
  transferMode.value === 'export'
    ? t('products.translation.transfer.allLanguagesExportHelp')
    : t('products.translation.transfer.allLanguagesImportHelp')
));

const hasSourceForCurrentLanguage = computed(() => !!sourcePreview.value);
const hasAnySourceLanguages = computed(() => sourceTranslations.value.length > 0);
const sourceLanguageBadges = computed(() =>
  Array.from(new Set(sourceTranslations.value.map((item) => item.language))).filter(Boolean),
);
const sourceMatchesTarget = computed(() =>
  !!sourceChannel.value && targetChannels.value.some((channelId) => channelId === sourceChannel.value),
);
const showMultiLanguageWarning = computed(() =>
  transferAllLanguages.value &&
  !!sourceChannel.value &&
  !hasSourceForCurrentLanguage.value &&
  hasAnySourceLanguages.value,
);

const canTransfer = computed(() => {
  if (transferring.value) return false;
  if (!hasSourceSelection.value || !hasTargetSelection.value) return false;
  if (sourceMatchesTarget.value) return false;
  if (!hasSelectedFields.value) return false;

  if (!isSingleUpdate.value) {
    if (transferAllLanguages.value) return true;
    return hasLanguageSelection.value;
  }

  if (transferAllLanguages.value) {
    return hasAnySourceLanguages.value;
  }

  return hasLanguageSelection.value && hasSourceForCurrentLanguage.value;
});

const resetSelectedFields = () => {
  transferFieldKeys.forEach((field) => {
    selectedFields[field] = baseAllowedFields.value[field];
  });
};

const updateSelectedField = (field: TransferFieldKey, value: boolean) => {
  selectedFields[field] = value;
};

const openModal = () => {
  if (!hasProducts.value) return;
  showModal.value = true;
};

const normalizeHtmlValue = (value?: string | null) => {
  if (!value) return emptyHtml;
  const trimmed = value.trim();
  return trimmed || emptyHtml;
};

const isEmptyHtml = (value?: string | null) => {
  if (!value) return true;
  const normalized = value.replace(/\s/g, '').toLowerCase();
  return normalized === '<p><br></p>' || normalized === '<br>' || normalized === '<p></p>';
};

const hasValue = (field: ContentTextFieldKey, value?: string | null) => {
  if (field === 'shortDescription' || field === 'description') {
    return !isEmptyHtml(value);
  }
  return !!value && value.trim().length > 0;
};

const resolvePreviewValue = (
  field: ContentTextFieldKey,
  sourceValue?: string | null,
  destinationValue?: string | null,
) => {
  if (!allowedFields.value[field]) {
    return field === 'shortDescription' || field === 'description'
      ? normalizeHtmlValue(destinationValue)
      : (destinationValue || '');
  }

  if (overrideExisting.value) {
    return field === 'shortDescription' || field === 'description'
      ? normalizeHtmlValue(sourceValue)
      : (sourceValue || '');
  }

  if (hasValue(field, destinationValue)) {
    return field === 'shortDescription' || field === 'description'
      ? normalizeHtmlValue(destinationValue)
      : (destinationValue || '');
  }

  if (hasValue(field, sourceValue)) {
    return field === 'shortDescription' || field === 'description'
      ? normalizeHtmlValue(sourceValue)
      : (sourceValue || '');
  }

  return field === 'shortDescription' || field === 'description'
    ? normalizeHtmlValue(destinationValue)
    : (destinationValue || '');
};

const previewContent = computed(() => {
  if (!sourceChannel.value) return null;
  if (!previewTargetChannel.value) return null;
  if (transferAllLanguages.value && !hasSourceForCurrentLanguage.value) return null;
  if (!sourcePreview.value && !destinationPreview.value) return null;

  const source = sourcePreview.value;
  const destination = destinationPreview.value;

  return {
    name: resolvePreviewValue('name', source?.name, destination?.name),
    subtitle: resolvePreviewValue('subtitle', source?.subtitle, destination?.subtitle),
    shortDescription: resolvePreviewValue('shortDescription', source?.shortDescription, destination?.shortDescription),
    description: resolvePreviewValue('description', source?.description, destination?.description),
    urlKey: resolvePreviewValue('urlKey', source?.urlKey, destination?.urlKey),
  };
});

const previewBulletPoints = computed(() => {
  if (!allowedFields.value.bulletPoints) {
    return destinationPreviewBulletPoints.value;
  }

  const destinationHasPoints = destinationPreviewBulletPoints.value.some(
    (point) => point.text && point.text.trim(),
  );

  if (overrideExisting.value) {
    return sourcePreviewBulletPoints.value;
  }

  if (destinationHasPoints) {
    return destinationPreviewBulletPoints.value;
  }

  return sourcePreviewBulletPoints.value;
});

const buildFieldPayload = (source: TranslationNode, destination: TranslationNode | null) => {
  const payload: Record<string, string> = {};

  textFieldKeys.forEach((field) => {
    if (!allowedFields.value[field]) return;
    const sourceValue = source[field] ?? null;
    const destinationValue = destination?.[field] ?? null;

    if (!overrideExisting.value) {
      if (hasValue(field, destinationValue)) return;
      if (!hasValue(field, sourceValue)) return;
      payload[field] = field === 'shortDescription' || field === 'description'
        ? normalizeHtmlValue(sourceValue)
        : (sourceValue || '');
      return;
    }

    payload[field] = field === 'shortDescription' || field === 'description'
      ? normalizeHtmlValue(sourceValue)
      : (sourceValue || '');
  });

  return payload;
};

const fetchTranslationsForChannel = async (channelId: string): Promise<TranslationNode[]> => {
  if (!resolvedProductId.value) return [];
  const query = channelId === 'default' ? getProductContentsByDefaultQuery : getProductContentsByChannelQuery;
  const variables = channelId === 'default'
    ? { productId: resolvedProductId.value }
    : { productId: resolvedProductId.value, salesChannelId: channelId };

  const { data } = await apolloClient.query({
    query,
    variables,
    fetchPolicy: 'network-only',
  });

  return data?.productTranslations?.edges?.map((edge: any) => edge.node as TranslationNode) ?? [];
};

const fetchBulletPoints = async (translationId: string) => {
  const { data } = await apolloClient.query({
    query: productTranslationBulletPointsQuery,
    variables: { filter: { productTranslation: { id: { exact: translationId } } } },
    fetchPolicy: 'network-only',
  });

  return data?.productTranslationBulletPoints?.edges?.map((edge: any) => edge.node) ?? [];
};

const loadLanguageOptions = async () => {
  if (props.currentLanguage) return;
  languageLoading.value = true;
  try {
    const { data } = await apolloClient.query({
      query: translationLanguagesQuery,
      fetchPolicy: 'cache-first',
    });

    const languages = (data?.translationLanguages?.languages || []) as LanguageOption[];
    languageOptions.value = languages.map((language) => ({
      code: language.code,
      name: language.name || language.code,
    }));

    const defaultCode = data?.translationLanguages?.defaultLanguage?.code || null;
    if (!selectedLanguage.value) {
      selectedLanguage.value = defaultCode || languageOptions.value[0]?.code || null;
    }
  } finally {
    languageLoading.value = false;
  }
};

const clearSourcePreview = () => {
  sourceTranslations.value = [];
  sourcePreview.value = null;
  sourcePreviewBulletPoints.value = [];
};

const clearDestinationPreview = () => {
  destinationTranslations.value = [];
  destinationPreview.value = null;
  destinationPreviewBulletPoints.value = [];
};

const loadSourceTranslations = async () => {
  if (!sourceChannel.value || !isSingleUpdate.value) {
    clearSourcePreview();
    return;
  }

  loading.value = true;
  try {
    sourceTranslations.value = await fetchTranslationsForChannel(sourceChannel.value);
  } finally {
    loading.value = false;
  }
};

const loadDestinationTranslations = async () => {
  if (!previewTargetChannel.value || !isSingleUpdate.value) {
    clearDestinationPreview();
    return;
  }
  destinationTranslations.value = await fetchTranslationsForChannel(previewTargetChannel.value);
};

const updatePreview = async () => {
  if (!resolvedLanguage.value || !sourceChannel.value || !previewTargetChannel.value || !isSingleUpdate.value) {
    sourcePreview.value = null;
    sourcePreviewBulletPoints.value = [];
    destinationPreview.value = null;
    destinationPreviewBulletPoints.value = [];
    return;
  }

  const language = resolvedLanguage.value;

  sourcePreview.value =
    sourceTranslations.value.find((item) => item.language === language) || null;
  sourcePreviewBulletPoints.value = [];

  destinationPreview.value =
    destinationTranslations.value.find((item) => item.language === language) || null;
  destinationPreviewBulletPoints.value = [];

  if (sourcePreview.value && sourceRules.value.bulletPoints) {
    sourcePreviewBulletPoints.value = await fetchBulletPoints(sourcePreview.value.id);
  }

  if (destinationPreview.value && previewDestinationRules.value.bulletPoints) {
    destinationPreviewBulletPoints.value = await fetchBulletPoints(destinationPreview.value.id);
  }
};

const refreshTransferData = async () => {
  await loadSourceTranslations();
  await loadDestinationTranslations();
  await updatePreview();
  resetSelectedFields();
};

const deleteBulletPoints = async (points: BulletPointNode[]) => {
  const payload = points
    .filter((point) => point.id)
    .map((point) => ({ id: point.id }));

  if (!payload.length) return;

  await apolloClient.mutate({
    mutation: deleteProductTranslationBulletPointsMutation,
    variables: { data: payload },
  });
};

const createBulletPoints = async (translationId: string, points: BulletPointNode[]) => {
  const payload = points
    .filter((point) => point.text && point.text.trim())
    .map((point, index) => ({
      text: point.text,
      sortOrder: point.sortOrder ?? index,
      productTranslation: { id: translationId },
    }));

  if (!payload.length) return;

  await apolloClient.mutate({
    mutation: createProductTranslationBulletPointsMutation,
    variables: { data: payload },
  });
};

const transferBulletPoints = async (
  sourceTranslationId: string,
  destinationTranslationId: string,
) => {
  const sourcePoints = await fetchBulletPoints(sourceTranslationId);
  const destinationPoints = await fetchBulletPoints(destinationTranslationId);
  const destinationHasPoints = destinationPoints.some((point) => point.text && point.text.trim());

  if (!overrideExisting.value && destinationHasPoints) return;

  if (destinationPoints.length) {
    await deleteBulletPoints(destinationPoints);
  }

  if (sourcePoints.length) {
    await createBulletPoints(destinationTranslationId, sourcePoints);
  }
};

const transferSingleToTarget = async (targetChannelId: string) => {
  if (!resolvedProductId.value) return;

  const normalizedSourceList: TranslationNode[] = (
    transferAllLanguages.value
      ? sourceTranslations.value
      : sourcePreview.value
        ? [sourcePreview.value]
        : []
  ).filter(Boolean) as TranslationNode[];

  const targetTranslations = await fetchTranslationsForChannel(targetChannelId);
  const targetMap = new Map(targetTranslations.map((item) => [item.language, item]));

  for (const sourceItem of normalizedSourceList) {
    if (!sourceItem.language) continue;
    const destinationItem = targetMap.get(sourceItem.language) || null;
    const payload = buildFieldPayload(sourceItem, destinationItem);
    const salesChannelInput = targetChannelId === 'default' ? null : { id: targetChannelId };

    let destinationId = destinationItem?.id || null;

    if (Object.keys(payload).length) {
      if (destinationId) {
        await apolloClient.mutate({
          mutation: updateProductTranslationMutation,
          variables: { data: { id: destinationId, ...payload } },
        });
      } else {
        const { data } = await apolloClient.mutate({
          mutation: createProductTranslationMutation,
          variables: {
            data: {
              product: { id: resolvedProductId.value },
              language: sourceItem.language,
              salesChannel: salesChannelInput,
              ...payload,
            },
          },
        });
        destinationId = data?.createProductTranslation?.id || null;
      }
    }

    if (allowedFields.value.bulletPoints && destinationId) {
      await transferBulletPoints(sourceItem.id, destinationId);
    }
  }
};

const handleTransferSingle = async () => {
  if (!canTransfer.value || !sourceChannel.value) return;
  if (!resolvedProductId.value) return;
  if (!hasLanguageSelection.value && !transferAllLanguages.value) return;

  transferring.value = true;
  try {
    for (const targetChannelId of targetChannels.value) {
      await transferSingleToTarget(targetChannelId);
    }

    Toast.success(t('products.translation.transfer.success'));
    showModal.value = false;
    emit('transferred');
    emit('imported');
  } catch (error) {
    const validationErrors = processGraphQLErrors(error, t);
    if (Object.keys(validationErrors).length) {
      Object.values(validationErrors).forEach((message) => Toast.error(message));
    } else {
      Toast.error(t('shared.messages.somethingWentWrong'));
    }
  } finally {
    transferring.value = false;
  }
};

const transferBulkToTarget = async (targetChannelId: string) => {
  if (!sourceChannel.value) return false;

  const channelSource = sourceChannel.value === 'default' ? null : { id: sourceChannel.value };
  const channelTarget = targetChannelId === 'default' ? null : { id: targetChannelId };
  const products = props.productIds.map((id) => ({ id: String(id) }));

  const { data } = await apolloClient.mutate({
    mutation: importProductTranslationsMutation,
    variables: {
      instance: {
        channelSource,
        channelTarget,
        language: transferAllLanguages.value ? null : resolvedLanguage.value,
        override: overrideExisting.value,
        allLanguages: transferAllLanguages.value,
        fields: selectedFieldEnums.value,
        products,
      },
    },
  });

  return Boolean(data?.importProductTranslations?.success);
};

const handleTransferMulti = async () => {
  if (!canTransfer.value || !sourceChannel.value) return;

  transferring.value = true;
  try {
    for (const targetChannelId of targetChannels.value) {
      const success = await transferBulkToTarget(targetChannelId);
      if (!success) {
        Toast.error(t('shared.messages.somethingWentWrong'));
        return;
      }
    }

    Toast.success(t('products.translation.transfer.multiStarted'));
    showModal.value = false;
    emit('transferred');
    emit('imported');
  } catch (error) {
    const validationErrors = processGraphQLErrors(error, t);
    if (Object.keys(validationErrors).length) {
      Object.values(validationErrors).forEach((message) => Toast.error(message));
    } else {
      Toast.error(t('shared.messages.somethingWentWrong'));
    }
  } finally {
    transferring.value = false;
  }
};

const handleTransfer = async () => {
  if (isSingleUpdate.value) {
    await handleTransferSingle();
    return;
  }
  await handleTransferMulti();
};

const resetTransferState = async () => {
  overrideExisting.value = false;
  transferAllLanguages.value = false;
  transferMode.value = 'import';
  selectedSourceChannel.value = null;
  selectedTargetChannel.value = props.currentSalesChannel ?? null;
  selectedTargetChannels.value = [];
  selectedLanguage.value = props.currentLanguage ?? null;
  clearSourcePreview();
  clearDestinationPreview();

  if (!props.currentLanguage) {
    await loadLanguageOptions();
  }

  await refreshTransferData();
};

watch(showModal, async (open) => {
  if (!open) return;
  await resetTransferState();
});

watch(transferMode, async () => {
  selectedSourceChannel.value = null;
  selectedTargetChannel.value = props.currentSalesChannel ?? null;
  selectedTargetChannels.value = [];
  clearSourcePreview();
  clearDestinationPreview();
  await refreshTransferData();
});

watch(
  () => sourceChannel.value,
  async (next) => {
    if (next) {
      selectedTargetChannels.value = selectedTargetChannels.value.filter((channelId) => channelId !== next);
    }
    await loadSourceTranslations();
    await updatePreview();
    resetSelectedFields();
  },
);

watch(
  () => importTargetChannel.value,
  async (next) => {
    if (transferMode.value === 'import' && next && selectedSourceChannel.value === next) {
      selectedSourceChannel.value = null;
    }
    await loadDestinationTranslations();
    await updatePreview();
    resetSelectedFields();
  },
);

watch(
  () => selectedTargetChannels.value.join('|'),
  async () => {
    if (sourceChannel.value) {
      selectedTargetChannels.value = selectedTargetChannels.value.filter((channelId) => channelId !== sourceChannel.value);
    }
    await loadDestinationTranslations();
    await updatePreview();
    resetSelectedFields();
  },
);

watch(
  () => resolvedLanguage.value,
  async () => {
    await updatePreview();
  },
);

const transferActionLabel = computed(() => {
  if (transferMode.value === 'export') {
    if (!targetChannels.value.length) return t('products.translation.transfer.exportAction');
    return t('products.translation.transfer.exportActionWithCount', { count: targetChannels.value.length });
  }

  if (!sourceChannel.value) return t('products.translation.transfer.importAction');
  return t('products.translation.transfer.importActionWithSource', { source: sourceChannelLabel.value });
});
</script>

<template>
  <div class="inline-flex items-center">
    <Button :class="buttonClass" :disabled="!hasProducts" @click="openModal">
      <Icon v-if="props.iconName" :name="props.iconName" size="sm" :class="props.iconClass" />
      {{ t(props.buttonLabel) }}
    </Button>

    <Modal v-model="showModal" @closed="showModal = false">
      <Card class="modal-content w-[80vw] max-w-5xl max-h-[80vh] overflow-y-auto">
        <div class="border-b border-gray-200 pb-4 mb-4">
          <h3 class="text-xl font-semibold">
            {{ t('products.translation.transfer.title') }}
          </h3>
          <p class="text-sm text-gray-500 mt-2">
            {{ isSingleUpdate ? t('products.translation.transfer.descriptionSingle') : t('products.translation.transfer.descriptionMulti') }}
          </p>
        </div>

        <div class="space-y-5">
          <div class="space-y-3">
            <div>
              <h4 class="text-sm font-semibold text-gray-800">
                {{ t('products.translation.transfer.modeHeading') }}
              </h4>
              <p class="text-xs text-gray-500 mt-1">
                {{ t('products.translation.transfer.modeHelp') }}
              </p>
            </div>

            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
              <button
                v-for="option in modeOptions"
                :key="option.key"
                type="button"
                class="group rounded-lg border p-4 text-left transition hover:border-primary hover:shadow-sm"
                :class="transferMode === option.key
                  ? 'border-primary bg-primary text-white shadow-sm'
                  : 'border-gray-200 bg-white text-gray-700'"
                @click="transferMode = option.key"
              >
                <div class="flex items-start gap-3">
                  <span
                    class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg"
                    :class="transferMode === option.key ? 'bg-white/15 text-white' : 'bg-blue-50 text-primary group-hover:bg-blue-100'"
                  >
                    <Icon :name="option.icon" size="sm" />
                  </span>
                  <span class="min-w-0">
                    <span class="block text-sm font-semibold">
                      {{ option.title }}
                    </span>
                    <span
                      class="mt-1 block text-sm leading-5"
                      :class="transferMode === option.key ? 'text-white/85' : 'text-gray-600'"
                    >
                      {{ option.description }}
                    </span>
                    <span
                      class="mt-2 block text-xs leading-5"
                      :class="transferMode === option.key ? 'text-white/75' : 'text-gray-500'"
                    >
                      {{ option.detail }}
                    </span>
                  </span>
                </div>
              </button>
            </div>
          </div>

          <div v-if="transferMode === 'import'" class="space-y-4">
            <div class="space-y-2">
              <label class="text-sm font-semibold text-gray-700">
                {{ t('products.translation.transfer.importTargetLabel') }}
              </label>
              <div
                v-if="props.currentSalesChannel !== null"
                class="flex items-center rounded-lg border border-gray-200 bg-gray-50 px-4 py-2 text-sm text-gray-700"
              >
                {{ targetChannelLabel }}
              </div>
              <IntegrationsSelector
                v-else
                v-model="selectedTargetChannel"
                :integrations="props.salesChannels"
                :add-default="true"
                :placeholder="t('products.translation.transfer.importTargetPlaceholder')"
                :removable="false"
              />
            </div>

            <div class="space-y-2">
              <label class="text-sm font-semibold text-gray-700">
                {{ t('products.translation.transfer.importSourceLabel') }}
              </label>
              <IntegrationsSelector
                v-model="selectedSourceChannel"
                :integrations="props.salesChannels"
                :exclude-ids="sourceExcludeIds"
                :add-default="sourceAddDefault"
                :placeholder="t('products.translation.transfer.importSourcePlaceholder')"
                :removable="false"
                :disabled="!sourceChannelCount"
              />
            </div>
          </div>

          <div v-else class="space-y-4">
            <div class="space-y-2">
              <label class="text-sm font-semibold text-gray-700">
                {{ t('products.translation.transfer.exportSourceLabel') }}
              </label>
              <div
                v-if="props.currentSalesChannel !== null"
                class="flex items-center rounded-lg border border-gray-200 bg-gray-50 px-4 py-2 text-sm text-gray-700"
              >
                {{ sourceChannelLabel }}
              </div>
              <IntegrationsSelector
                v-else
                v-model="selectedSourceChannel"
                :integrations="props.salesChannels"
                :exclude-ids="sourceExcludeIds"
                :add-default="sourceAddDefault"
                :placeholder="t('products.translation.transfer.exportSourcePlaceholder')"
                :removable="false"
                :disabled="!sourceChannelCount"
              />
            </div>

            <div class="space-y-2">
              <div class="flex items-center justify-between gap-3">
                <label class="text-sm font-semibold text-gray-700">
                  {{ t('products.translation.transfer.exportTargetLabel') }}
                </label>
                <Badge :text="exportTargetsBadge" color="blue" />
              </div>
              <IntegrationsSelector
                v-model="selectedTargetChannels"
                :integrations="props.salesChannels"
                :exclude-ids="targetExcludeIds"
                :add-default="targetAddDefault"
                :placeholder="t('products.translation.transfer.exportTargetPlaceholder')"
                :removable="false"
                :disabled="!targetChannelCount"
                multiple
              />
            </div>
          </div>

          <div v-if="showLanguageSelector" class="space-y-2">
            <label class="text-sm font-semibold text-gray-700">
              {{ t('shared.labels.language') }}
            </label>
            <Selector
              v-model="selectedLanguage"
              :options="languageOptions"
              label-by="name"
              value-by="code"
              :placeholder="t('products.translation.placeholders.language')"
              :removable="false"
              :is-loading="languageLoading"
              :disabled="languageLoading || !languageOptions.length"
            />
          </div>

          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <label class="flex items-center justify-between gap-4 rounded-lg border border-gray-200 px-4 py-3">
              <div>
                <div class="text-sm font-medium text-gray-800">
                  {{ t('products.translation.transfer.overrideLabel') }}
                </div>
                <div class="text-xs text-gray-500">
                  {{ t('products.translation.transfer.overrideHelp') }}
                </div>
              </div>
              <Toggle v-model="overrideExisting" />
            </label>

            <label class="flex items-center justify-between gap-4 rounded-lg border border-gray-200 px-4 py-3">
              <div>
                <div class="text-sm font-medium text-gray-800">
                  {{ t('products.translation.transfer.allLanguagesLabel') }}
                </div>
                <div class="text-xs text-gray-500">
                  {{ allLanguagesHelp }}
                </div>
                <div v-if="transferAllLanguages" class="mt-2 flex flex-wrap gap-2">
                  <Badge v-if="isSingleUpdate" :text="availableLanguagesBadge" color="indigo" />
                  <Badge
                    v-if="isSingleUpdate"
                    v-for="code in sourceLanguageBadges"
                    :key="code"
                    :text="code"
                    color="green"
                  />
                </div>
                <div v-else class="mt-2 flex flex-wrap gap-2">
                  <Badge :text="currentLanguageBadge" color="gray" />
                </div>
              </div>
              <Toggle v-model="transferAllLanguages" />
            </label>
          </div>

          <Accordion :items="advancedOptionsItems">
            <template #fields>
              <div class="space-y-4">
                <div class="space-y-1">
                  <h5 class="text-sm font-semibold text-gray-700">
                    {{ t('products.translation.transfer.fieldsLabel') }}
                  </h5>
                  <p class="text-xs text-gray-500">
                    {{ t('products.translation.transfer.fieldsHelp') }}
                  </p>
                </div>
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <label
                    v-for="field in fieldOptions"
                    :key="field.key"
                    class="flex items-center rounded-md border border-gray-200 px-3 py-2"
                    :class="field.available ? '' : 'cursor-not-allowed opacity-50'"
                  >
                    <Checkbox
                      :model-value="selectedFields[field.key]"
                      :disabled="!field.available"
                      @update:modelValue="(value) => updateSelectedField(field.key, value)"
                    >
                      {{ field.label }}
                    </Checkbox>
                  </label>
                </div>
              </div>
            </template>
          </Accordion>

          <div v-if="isSingleUpdate" class="space-y-2">
            <div class="flex items-center justify-between">
              <h4 class="text-sm font-semibold text-gray-700">
                {{ previewTitle }}
              </h4>
            </div>

            <div v-if="loading" class="text-sm text-gray-500">
              {{ t('shared.labels.loading') }}
            </div>

            <div
              v-else-if="sourceChannel && !hasSourceForCurrentLanguage && !transferAllLanguages"
              class="rounded-md border border-red-200 bg-red-50 px-4 py-2 text-sm text-red-700"
            >
              {{ t('products.translation.transfer.missingSource') }}
            </div>

            <div
              v-else-if="showMultiLanguageWarning"
              class="rounded-md border border-orange-200 bg-orange-50 px-4 py-2 text-sm text-orange-700"
            >
              {{ t('products.translation.transfer.missingSourceMultiLanguage') }}
            </div>

            <div v-else-if="sourceChannel && previewContent" class="rounded-lg border border-gray-200">
              <ProductContentPreview
                :content="previewContent"
                :default-content="null"
                :current-channel="previewChannel"
                :channels="props.salesChannels"
                :bullet-points="previewBulletPoints"
              />
            </div>
          </div>

          <div class="flex items-center justify-end gap-3 pt-2">
            <Button class="btn btn-outline-dark" @click="showModal = false">
              {{ t('shared.button.cancel') }}
            </Button>
            <Button class="btn btn-primary" :disabled="!canTransfer" @click="handleTransfer">
              {{ transferActionLabel }}
            </Button>
          </div>
        </div>
      </Card>
    </Modal>
  </div>
</template>
