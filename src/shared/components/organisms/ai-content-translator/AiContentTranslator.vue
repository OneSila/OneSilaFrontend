...
const onContentTranslated = (data: any) => {
  if (progressInterval) {
    clearInterval(progressInterval);
    progressInterval = null;
  }
  ...
  const translatedText = data?.data?.generateAiTranslation?.content;
  if (translatedText) {
    emit("translated", translatedText);
  }
  // Optionally, show points if provided.
  const points = data?.data?.generateAiTranslation?.points;
  if (points) {
    Toast.success(
      t('shared.components.organisms.aiContentTranslator.success', { points: points })
    );
  }

  const errorMessages = data?.data?.generateAiTranslation?.messages || [];
...
<template>
  <ApolloMutation
    ...
    @done="onContentTranslated"
    @error="onGenerateError"
  >
    <template v-slot="{ mutate, loading }">
      <div class="relative">
        <Button class="btn btn-sm btn-outline-success" @click="() => handleClick(mutate)">
        ...
        <div class="fixed inset-0 flex items-center justify-center z-50">
          <div class="bg-gray-50 rounded-lg py-6 px-10 shadow-lg w-96">
            <h3 class="text-lg text-center font-bold mb-2">
              {{ t("shared.components.organisms.aiContentTranslator.title") }}
            </h3>
            <hr />
            <ul class="mt-2">
              <li
